...
export const CompleteProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      setFormData(data);
      const hasAutoGenerated =
        data.display_name.startsWith('Wallet ') ||
        data.username.startsWith('user_') ||
        data.email.includes('@temp.thirdweb');

      if (hasAutoGenerated) {
        setTouched({ display_name: true, username: true, email: true });
        const newErrors: Partial<CompleteProfileDto> = {};
        (Object.keys(data) as Array<keyof CompleteProfileDto>).forEach((field) => {
  ...
  const validateField = (
    ...
    switch (field) {
      case 'display_name':
        ...
        if (value.trim().startsWith('Wallet '))
          return 'Debes ingresar tu nombre real';
      ...
      case 'username':
        ...
        if (value.trim().startsWith('user_'))
          return 'Debes elegir un nombre de usuario personalizado';
      ...
      case 'email':
        ...
        if (value.includes('@temp.thirdweb'))
